...
export function ServiceCard({
  ...
  const latency = useServiceLatency(
    service.port,
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs cursor-pointer transition-all hover:border-primary/50 hover:bg-accent/30 select-none ${
          hasConflict ? "border-yellow-500 bg-yellow-500/5" : "bg-card"
        } ${isPreview ? "group-hover:ring-2 group-hover:ring-blue-400/40 group-hover:ring-offset-1" : ""}`}
      >
        {showChecking ? (
          ...
          <span
            className={`inline-block h-2 w-2 rounded-full ${statusDot[service.status]}`}
          />
        )}
      ...
      {hover && (
        <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-1.5 pointer-events-none">
          <div className="rounded-md border bg-popover px-3 py-2 text-xs shadow-md whitespace-nowrap">
            ...
            {service.description && (
              <div className="text-muted-foreground mt-0.5 max-w-48">
                {service.description}
              </div>
            )}
