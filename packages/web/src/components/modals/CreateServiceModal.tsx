...
import type { CreateServiceBody } from '@portfolio/shared';
...
interface Props {
  ...
  onSave: (body: CreateServiceBody) => Promise<any>;
}

export function CreateServiceModal({ open, projectName, onClose, onSave }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="w-full max-w-md rounded-lg border bg-card p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-semibold mb-1">{t('service.add')}</h2>
        <p className="text-sm text-muted-foreground mb-4">{t('service.to', { project: projectName })}</p>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('service.namePlaceholder')} autoFocus />
          </div>
          <div>
            <label className="text-sm font-medium">{t('service.portRange')}</label>
            <input type="number" className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm" value={port} onChange={(e) => setPort(e.target.value)} placeholder={t('service.portPlaceholder')} />
          </div>
          <div>
